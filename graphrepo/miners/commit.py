...
class CommitMiner(DefaultMiner):
    ...
    def get_parents(self, commit_hash):
        """Returns the parent commits of the commit
        :param commit_hash: optional; if given, it will
          return the data only for one commit
        ...
        query = """
          ...
          return distinct p
          """.format(commit_hash)
        parents_ = self.graph.run(query)
        return [x['p'] for x in parents_.data()]
  
    def get_commit_devs(self, commit_hash):
        """Returns the developers linked to a commit
        :param commit_hash: optional; if given, it will
          return the data only for one commit
        ...
        query = """
          ...
          return distinct d
          """.format(commit_hash)
        devs_ = self.graph.run(query)
        return [x['d'] for x in devs_.data()]
